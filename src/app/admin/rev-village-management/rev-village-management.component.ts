...
@Component({
  selector: 'app-rev-village-management',
  templateUrl: './rev-village-management.component.html',
  styleUrls: ['./rev-village-management.component.scss']
})
export class RevVillageManagementComponent {
  ...
  search() {
    ...
    const payload = {
      ...
      "FILTERS": {
        ...
        "TALUK_ID": '',
        "REV_VILLAGE_NAME": this.StreetForm.value.hud_name ? this.StreetForm.value.hud_name : '',
        "REV_VILLAGE_GID": this.StreetForm.value.hud_gid ? this.StreetForm.value.hud_gid : null,
      },
      "LIMIT": 10,
      "OFFSET": 0
    }
    ...
    this.adminService.getRevVillage(payload).subscribe((data: any) => {
      ...
      this.data = data.data.map((item: any) => {
        ...
        item.villagename = `${item.rev_village_name}`.trim()
        item.villagegid = `${item.rev_village_gid}`.trim()
        return item;
      });
  ...
  createUser() {
    this.router.navigate(['/admin/revvillage-add/0']);
  }
  createForm() {
  ...
  edit(value: any) {
    this.router.navigate(['admin/revvillage-add/' + value.village_gid + '']);
  }
  clearSearch() {
